    ...
    bar.innerHTML += `
      <div class="filter-group filter-search">
        ...
        <div class="agency-search">
          <svg class="agency-search-icon" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
          <input type="text" class="agency-search-input" placeholder="Rechercher parmi 20 agences..." id="agencySearch" autocomplete="off">
          <span class="agency-search-count">20</span>
          <div class="agency-search-results" id="agencyResults" hidden>
            <div class="agency-search-item" data-id="all"><strong>Toutes les agences</strong><span>20 agences · 308 artisans</span></div>
            <div class="agency-search-item" data-id="poitiers"><strong>Poitiers Centre</strong><span>Vienne · 28 artisans · M. Valentin Morel</span></div>
            <div class="agency-search-item" data-id="niort"><strong>Niort</strong><span>Deux-Sèvres · 22 artisans · Mme Claire Petit</span></div>
            <div class="agency-search-item" data-id="larochelle"><strong>La Rochelle</strong><span>Charente-Maritime · 12 artisans · Mme Isabelle Lefèvre</span></div>
            <div class="agency-search-item" data-id="angouleme"><strong>Angoulême</strong><span>Charente · 18 artisans · Mme Sophie Giraud</span></div>
            <div class="agency-search-item" data-id="saintes"><strong>Saintes</strong><span>Charente-Maritime · 14 artisans · M. Julien Faure</span></div>
          </div>
        </div>
      </div>
    `;
...
document.addEventListener('input', (e) => {
  if (!e.target.classList.contains('agency-search-input')) return;
  const q = e.target.value.trim().toLowerCase();
  const results = e.target.parentElement.querySelector('.agency-search-results');
  if (!results) return;
  results.hidden = false;
  results.querySelectorAll('.agency-search-item').forEach(it => {
    const txt = it.textContent.toLowerCase();
    it.style.display = txt.includes(q) ? '' : 'none';
  });
});
document.addEventListener('focusin', (e) => {
  if (e.target.classList.contains('agency-search-input')) {
    const results = e.target.parentElement.querySelector('.agency-search-results');
...
document.addEventListener('click', (e) => {
  // pick item
  const item = e.target.closest('.agency-search-item');
  if (item) {
    const search = item.closest('.agency-search');
    const input = search.querySelector('.agency-search-input');
    input.value = item.querySelector('strong').textContent;
    search.querySelector('.agency-search-results').hidden = true;
    return;
  }
  // close on outside click
  if (!e.target.closest('.agency-search')) {
    document.querySelectorAll('.agency-search-results').forEach(r => r.hidden = true);
  }
});
